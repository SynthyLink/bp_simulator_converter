...
{
    internal partial  class IBSampleAppDialog : Form
    {
        ...
        internal ExtendedClient ibClient;
        ...
        public IBSampleAppDialog()
        {
            ...
            ibClient = new ExtendedClient(signal);
            marketDataManager = new MarketDataManager(ibClient.Client, marketDataGrid_MDT);
            deepBookManager = new DeepBookManager(ibClient.Client, deepBookGrid, mktDepthExchangesGrid_MDT);
            historicalDataManager = new HistoricalDataManager(ibClient.Client, historicalChart, barsGrid);
            realTimeBarManager = new RealTimeBarsManager(ibClient.Client, rtBarsChart, rtBarsGrid);
            scannerManager = new ScannerManager(ibClient.Client, scannerGrid, scannerParamsOutput);
            orderManager = new OrderManager(ibClient.Client, liveOrdersGrid, completedOrdersGrid, tradeLogGrid);
            accountManager = new AccountManager(ibClient.Client, accountSelector, accSummaryGrid, accountValuesGrid, accountPortfolioGrid, positionsGrid, familyCodesGrid);
            contractManager = new ContractManager(ibClient.Client, fundamentalsOutput, contractDetailsGrid, bondContractDetailsGrid, comboBoxMarketRuleId, dataGridViewMarketRule, labelMarketRuleIdRes);
            advisorManager = new AdvisorManager(ibClient.Client, advisorAliasesGrid, advisorGroupsGrid, advisorProfilesGrid);
            optionsManager = new OptionsManager(ibClient.Client, optionChainCallGrid, optionChainPutGrid, optionPositionsGrid, listViewOptionParams);
            acctPosMultiManager = new AcctPosMultiManager(ibClient.Client, positionsMultiGrid, accountUpdatesMultiGrid);
            symbolSamplesManagerData = new SymbolSamplesManager(ibClient.Client, symbolSamplesDataGridData);
            symbolSamplesManagerContractInfo = new SymbolSamplesManager(ibClient.Client, symbolSamplesDataGridContractInfo);
            newsManager = new NewsManager(ibClient.Client, dataGridViewNewsTicks, dataGridViewNewsProviders, textBoxNewsArticle, dataGridViewHistoricalNews);
            pnlMgr = new PnLManager(ibClient.Client);
            wshMgr = new WshManager(ibClient.Client);
            ...
            ibClient.Client.Error += Client_Error;
            ibClient.Client.ConnectionClosed += Client_ConnectionClosed;
            ibClient.Client.CurrentTime += time => addTextToBox("Current Time: " + time + Environment.NewLine);
            ibClient.Client.TickPrice += Client_Tick;
            ibClient.Client.TickSize += Client_Tick;
            ibClient.Client.TickString += (tickerId, tickType, value) => addTextToBox("Tick string. Ticker Id:" + tickerId + ", Type: " + TickType.getField(tickType) + ", Value: " + value + Environment.NewLine);
            ibClient.Client.TickGeneric += Client_Tick;
            ibClient.Client.TickEFP += (tickerId, tickType, basisPoints, formattedBasisPoints, impliedFuture, holdDays, futureLastTradeDate, dividendImpact, dividendsToLastTradeDate) => addTextToBox("TickEFP. " + tickerId + ", Type: " + tickType + ", BasisPoints: " + Util.DoubleMaxString(basisPoints) + ", FormattedBasisPoints: " + formattedBasisPoints + ", ImpliedFuture: " + Util.DoubleMaxString(impliedFuture) + ", HoldDays: " + Util.IntMaxString(holdDays) + ", FutureLastTradeDate: " + futureLastTradeDate + ", DividendImpact: " + Util.DoubleMaxString(dividendImpact) + ", DividendsToLastTradeDate: " + Util.DoubleMaxString(dividendsToLastTradeDate) + Environment.NewLine);
            ibClient.Client.TickSnapshotEnd += tickerId => addTextToBox("TickSnapshotEnd: " + tickerId + Environment.NewLine);
            ibClient.Client.NextValidId += UpdateUI;
            ibClient.Client.DeltaNeutralValidation += (reqId, deltaNeutralContract) =>
            ...
            ibClient.Client.ManagedAccounts += UpdateUI;
            ibClient.Client.TickOptionCommunication += HandleTickMessage;

            ibClient.Client.AccountSummary += accountManager.HandleAccountSummary;
            ibClient.Client.AccountSummaryEnd += UpdateUI;
            ibClient.Client.UpdateAccountValue += accountManager.HandleAccountValue;
            ibClient.Client.UpdatePortfolio += UpdateUI;

            ibClient.Client.UpdateAccountTime += message => accUpdatesLastUpdateValue.Text = message.Timestamp;
            //ibClient.Client.AccountDownloadEnd += (do nothing)
            ibClient.Client.OrderStatus += orderManager.HandleOrderStatus;

            ibClient.Client.OpenOrder += orderManager.handleOpenOrder;
            //ibClient.Client.OpenOrderEnd += (do nothing)
            ibClient.Client.ContractDetails += HandleContractDataMessage;
            ibClient.Client.ContractDetailsEnd += reqId => UpdateUI(new ContractDetailsEndMessage());
            ibClient.Client.ExecDetails += orderManager.HandleExecutionMessage;
            ibClient.Client.ExecDetailsEnd += reqId => addTextToBox("ExecDetailsEnd. " + reqId + Environment.NewLine);
            ibClient.Client.CommissionReport += commissionReport => orderManager.HandleCommissionMessage(new CommissionMessage(commissionReport));
            ibClient.Client.FundamentalData += UpdateUI;

            ibClient.Client.HistoricalData += historicalDataManager.UpdateUI;
            ibClient.Client.HistoricalDataUpdate += historicalDataManager.UpdateUI;
            ibClient.Client.HistoricalDataEnd += historicalDataManager.UpdateUI;
            ibClient.Client.HistoricalDataEnd += historicalDataManager.SaveHistoricalData;


            ibClient.Client.MarketDataType += UpdateUI;
            ibClient.Client.UpdateMktDepth += deepBookManager.UpdateUI;
            ibClient.Client.UpdateMktDepthL2 += deepBookManager.UpdateUI;
            ibClient.Client.UpdateNewsBulletin += (msgId, msgType, message, origExchange) =>
                addTextToBox("News Bulletins. " + msgId + " - Type: " + msgType + ", Message: " + message + ", Exchange of Origin: " + origExchange + Environment.NewLine);

            ibClient.Client.Position += accountManager.HandlePosition;
            ibClient.Client.PositionEnd += () => addTextToBox("PositionEnd" + Environment.NewLine);
            ibClient.Client.RealtimeBar += realTimeBarManager.UpdateUI;
            ibClient.Client.ScannerParameters += xml => scannerManager.UpdateUI(new ScannerParametersMessage(xml));
            ibClient.Client.ScannerParameters += UpdateUi;
            ibClient.Client.ScannerData += scannerManager.UpdateUI;

            ibClient.Client.ScannerDataEnd += reqId => addTextToBox("ScannerDataEnd. " + reqId + Environment.NewLine);
            ibClient.Client.ReceiveFA += advisorManager.UpdateUI;
            ibClient.Client.ReplaceFAEnd += (reqId, text) => addTextToBox("Replace FA End. ReqId: " + reqId + ", Text: " + text + Environment.NewLine);
            ibClient.Client.BondContractDetails += contractManager.HandleBondContractMessage;
            ibClient.Client.VerifyMessageAPI += apiData => addTextToBox("verifyMessageAPI: " + apiData);
            ibClient.Client.VerifyCompleted += (isSuccessful, errorText) => addTextToBox("verifyCompleted. IsSuccessfule: " + isSuccessful + " - Error: " + errorText);
            ibClient.Client.VerifyAndAuthMessageAPI += (apiData, xyzChallenge) => addTextToBox("verifyAndAuthMessageAPI: " + apiData + " " + xyzChallenge);
            ibClient.Client.VerifyAndAuthCompleted += (isSuccessful, errorText) => addTextToBox("verifyAndAuthCompleted. IsSuccessfule: " + isSuccessful + " - Error: " + errorText);
            ibClient.Client.DisplayGroupList += (reqId, groups) => addTextToBox("DisplayGroupList. Request: " + reqId + ", Groups" + groups);
            ibClient.Client.DisplayGroupUpdated += (reqId, contractInfo) => addTextToBox("displayGroupUpdated. Request: " + reqId + ", ContractInfo: " + contractInfo);

            ibClient.Client.PositionMulti += acctPosMultiManager.HandlePositionMulti;
            ibClient.Client.PositionMultiEnd += reqId => acctPosMultiManager.HandlePositionMultiEnd(new PositionMultiEndMessage(reqId));
            ibClient.Client.AccountUpdateMulti += acctPosMultiManager.HandleAccountUpdateMulti;
            ibClient.Client.AccountUpdateMultiEnd += reqId => acctPosMultiManager.HandleAccountUpdateMultiEnd(new AccountUpdateMultiEndMessage(reqId));
            ibClient.Client.SecurityDefinitionOptionParameter += optionsManager.UpdateUI;
            //ibClient.Client.SecurityDefinitionOptionParameterEnd += (do nothing)
            ibClient.Client.SoftDollarTiers += orderManager.HandleSoftDollarTiers;
            ibClient.Client.FamilyCodes += familyCodes => accountManager.HandleFamilyCodes(new FamilyCodesMessage(familyCodes));
            ibClient.Client.SymbolSamples += UpdateUI;
            ibClient.Client.MktDepthExchanges += depthMktDataDescriptions => deepBookManager.HandleMktDepthExchangesMessage(new MktDepthExchangesMessage(depthMktDataDescriptions));
            ibClient.Client.TickNews += newsManager.UpdateUI;
            ibClient.Client.TickReqParams += UpdateUI;
            ibClient.Client.SmartComponents += (reqId, theMap) => theMap.ToList().ForEach(i => dataGridViewSmartComponents.Rows.Add(new object[] { i.Key, i.Value.Key, i.Value.Value }));
            ibClient.Client.NewsProviders += newsProviders => newsManager.HandleNewsProviders(new NewsProvidersMessage(newsProviders));
            ibClient.Client.NewsArticle += newsManager.UpdateUI;
            ibClient.Client.HistoricalNews += newsManager.UpdateUI;
            ibClient.Client.HistoricalNewsEnd += newsManager.UpdateUI;
            ibClient.Client.HeadTimestamp += UpdateUI;
            ibClient.Client.HistogramData += UpdateUI;
            ibClient.Client.RerouteMktDataReq += (reqId, conId, exchange) => addTextToBox("Re-route market data request. ReqId: " + reqId + ", ConId: " + conId + ", Exchange: " + exchange + Environment.NewLine);
            ibClient.Client.RerouteMktDepthReq += (reqId, conId, exchange) => addTextToBox("Re-route market depth request. ReqId: " + reqId + ", ConId: " + conId + ", Exchange: " + exchange + Environment.NewLine);
            ibClient.Client.MarketRule += contractManager.HandleMarketRuleMessage;
            ibClient.Client.pnl += msg => pnldataTable.Rows.Add(Util.DoubleMaxString(msg.DailyPnL), Util.DoubleMaxString(msg.UnrealizedPnL), Util.DoubleMaxString(msg.RealizedPnL));
            ibClient.Client.pnlSingle += msg => pnlSingledataTable.Rows.Add(Util.DecimalMaxString(msg.Pos), Util.DoubleMaxString(msg.DailyPnL), Util.DoubleMaxString(msg.UnrealizedPnL), Util.DoubleMaxString(msg.RealizedPnL), Util.DoubleMaxString(msg.Value));
            ibClient.Client.historicalTick += UpdateUI;
            ibClient.Client.historicalTickBidAsk += UpdateUI;
            ibClient.Client.historicalTickLast += UpdateUI;
            ibClient.Client.tickByTickAllLast += UpdateUI;
            ibClient.Client.tickByTickBidAsk += UpdateUI;
            ibClient.Client.tickByTickMidPoint += UpdateUI;
            ibClient.Client.OrderBound += msg => addTextToBox("Order bound. OrderId: " + Util.LongMaxString(msg.OrderId) + ", ApiClientId: " + Util.IntMaxString(msg.ApiClientId) + ", ApiOrderId: " + Util.IntMaxString(msg.ApiOrderId));
            ibClient.Client.CompletedOrder += orderManager.handleCompletedOrder;
            ibClient.Client.WshMetaData += (reqId, dataJson) => wshMetaDataTable.Rows.Add(reqId, dataJson);
            ibClient.Client.WshEventData += (reqId, dataJson) => wshEventDataTable.Rows.Add(reqId, dataJson);
            ibClient.Client.HistoricalSchedule += UpdateUI;
            //ibClient.Client.CompletedOrderEnd += (do nothing)
            ibClient.Client.UserInfo += whiteBrandingId => ShowMessageOnPanel("User Info. White Branding Id: " + whiteBrandingId);
        }
        ...
        void Client_Tick(TickSizeMessage msg)
        {
        ...
        void Client_Tick(TickPriceMessage msg)
        {
        ...
        void Client_Tick(TickGenericMessage msg)
        {
        ...
        void Client_Error(int id, int errorCode, string str, string advancedOrderRejectjson, Exception ex)
        {
            if (ex != null)
            {
        ...
        private void UpdateUI(ConnectionStatusMessage statusMessage)
        {
            ...
            {
                status_CT.Text = "Connected! Your client Id: " + ibClient.Client.ClientId;
        ...
        private void connectButton_Click(object sender, EventArgs e)
        {
            if (!IsConnected)
            {
                ...
                try
                {
                    ...
                    ibClient.Client.ClientId = int.Parse(clientid_CT.Text);
                    ibClient.Client.ClientSocket.eConnect(host, port, ibClient.Client.ClientId);

                    var reader = new EReader(ibClient.Client.ClientSocket, signal);

                    reader.Start();

                    new Thread(() => { while (ibClient.Client.ClientSocket.IsConnected()) { signal.waitForSignal(); reader.processMsgs(); } }) { IsBackground = true }.Start();
                }
                catch (Exception)
                {
        ...
        internal void WriteHisory()
        {
            if (!IsConnected)
            {
                return;
            }
            var state = StaticExtensionIBApi.HistoryWriteState;
            if (state == HistoryWriteState.Stopping)
            {
                return;
            }
            if (state == HistoryWriteState.Active)
            {
                StaticExtensionIBApi.HistoryWriteState = HistoryWriteState.Stopping;
                StaticExtensionIBApi.IndicateHistoryWriteState();
                return;
            }
            else
            {
                StaticExtensionIBApi.HistoryWriteState = HistoryWriteState.Active;
                StaticExtensionIBApi.IndicateHistoryWriteState();
            }
            var current = DateTime.Now;
            int kd = 10;
            var sym = symbol_TMD_MDT.Text;
            string dataDir = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "../Data"));
            var di = new DirectoryInfo(dataDir + Path.DirectorySeparatorChar + sym);
            if (!di.Exists)
            {
                di.Create();
            }
            ...
            bool first = true;
            foreach (var f in di.GetFiles("*.history"))
            {
            ...
            {
                list.Sort();
                current = list[0] + new TimeSpan(5, 0, 0, 0);
                first = false;
            }


            StaticExtensionIBSample.HistoryPath = di.FullName + Path.DirectorySeparatorChar;

            ThreadStart st = () =>
                {
                    while (true)
                    {
                        if (StaticExtensionIBApi.HistoryWriteState ==  HistoryWriteState.Stopping)
                        {
                            StaticExtensionIBApi.HistoryWriteState = HistoryWriteState.Stopped;
                            StaticExtensionIBApi.IndicateHistoryWriteState();
                            StaticExtensionIBSample.ResetHisoryWrite.Set();
                            return;
                        }
                        StaticExtensionIBSample.ResetHisoryWrite.Reset();
                        Contract contract = GetMDContract();
                        var end = current;
                        /*if (!first)
                        {
                        ...
                        string endTime = end.Convert();
                        string duration = (kd + 1) + " D";
                        string barSize = "1 min";// hdRequest_BarSize.Text.Trim();
                        string whatToShow = "";
                        int outsideRTH = 0;
                        bool check = false;
                        var act = () =>
                        {
                            whatToShow = hdRequest_WhatToShow.Text.Trim();
                            outsideRTH = contractMDRTH.Checked ? 1 : 0;
                            check = cbKeepUpToDate.Checked;
                            historicalDataManager.AddRequest(contract, endTime, duration,
                                        barSize, whatToShow, outsideRTH, 1, check);

                        };
                        this.InvokeIfNeeded(act);
                        StaticExtensionIBSample.ResetHisoryWrite.WaitOne();
                        current = current.AddDays(-kd);
                    }
                };

            new Thread(st) { IsBackground = true }.Start();
        }


        internal void HistData()
        {
            if (IsConnected)
            {
                Contract contract = GetMDContract();
        ...
        private void realTime_Button_Click(object sender, EventArgs e)
        {
            if (IsConnected)
            {
                ibClient.Add(new EWrapper[] {  Strategy1.Instance });
                Contract contract = GetMDContract();
                string whatToShow = hdRequest_WhatToShow.Text.Trim();
        ...
        private void refreshOrdersButton_Click(object sender, EventArgs e)
        {
            ...
            ibClient.Client.ClientSocket.reqAllOpenOrders();
        }
        ...
        private void refreshExecutionsButton_Click(object sender, EventArgs e)
        {
            ...
            ibClient.Client.ClientSocket.reqExecutions(1, execFilter);
        }

        private void bindOrdersButton_Click(object sender, EventArgs e)
        {
            ibClient.Client.ClientSocket.reqAutoOpenOrders(true);
        }
        ...
        {
            liveOrdersGrid.Rows.Clear();
            ibClient.Client.ClientSocket.reqOpenOrders();
        }

        private void globalCancelButton_Click(object sender, EventArgs e)
        {
            ibClient.Client.ClientSocket.reqGlobalCancel();
        }
        ...
        private void ReqSmartComponents_Button_Click(object sender, EventArgs e)
        {
            if (IsConnected)
            {
                ibClient.Client.ClientSocket.reqSmartComponents(new Random(DateTime.Now.Millisecond).Next(), bboExchange_comboBox.SelectedItem + "");
        ...
        private void headTimestamp_button_Click(object sender, EventArgs e)
        {
            if (IsConnected)
            {
                ...
                ibClient.Client.ClientSocket.reqHeadTimestamp(reqId, contract, whatToShow, 1, 1);
        ...
        private void histogram_button_Click(object sender, EventArgs e)
        {
            if (IsConnected)
            {
                ...
                ibClient.Client.ClientSocket.reqHistogramData(reqId, contract, true, duration);
        ...
        private void histogramClearLinkLabel_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            ibClient.Client.ClientSocket.cancelHistogramData(reqId);
        ...
        private void buttonReqMarketRule_Click(object sender, EventArgs e)
        {
            if (IsConnected)
            {
                ...
                ibClient.Client.ClientSocket.reqMarketRule(marketRuleId);
        ...
        private void btnRequestHistoricalTicks_Click(object sender, EventArgs e)
        {
            ...
            ibClient.Client.ClientSocket.reqHistoricalTicks(reqId, contract, tbStartDate.Text, tbEndDate.Text, numOfTicks, cbWhatToShow.Text,
        ...
        private void reqUserInfo_Click(object sender, EventArgs e)
        {
            if (IsConnected)
            {
                ibClient.Client.ClientSocket.reqUserInfo(0);
            }
        }
        ...
        private void buttonRequestTickByTick_Click(object sender, EventArgs e)
        {
            ...
            ibClient.Client.ClientSocket.reqTickByTickData(0, contract, tickType, numberOfTicks, cbIgnoreSize.Checked);
        }

        private void buttonCancelTickByTick_Click(object sender, EventArgs e)
        {
            ibClient.Client.ClientSocket.cancelTickByTickData(0);
        }
        ...
        private void completedOrdersButton_Click(object sender, EventArgs e)
        {
            ...
            ibClient.Client.ClientSocket.reqCompletedOrders(false);
        }

   
        internal FormAdditionalOptions additionalOptions = null;

        private void buttonAdditionalForm_Click(object sender, EventArgs e)
        {
            if (additionalOptions != null)
            {
                if (!additionalOptions.IsDisposed)
                {
                    additionalOptions.BringToFront();
                }
                return;
            }
            additionalOptions = new FormAdditionalOptions(this);
            additionalOptions.Show(this);
            additionalOptions.BringToFront();
        }
