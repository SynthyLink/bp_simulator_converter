...
exports.GLTFHelper = void 0;
const auxiliaries_1 = require("../auxiliaries");
class GLTFHelper {
    static modeToEnum(gl, mode) {
        if (mode < 0 || mode > 6) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Error, `Specified draw mode is ${mode} but is required to be between 0 and 6`);
        }
        if (mode === 0) {
            return gl.POINTS;
        }
     ...
     */
    static nameToAttributeIndex(name) {
        if (name === 'POSITION') {
            return 0;
        }
        if (name === 'NORMAL') {
        ...
        (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `Unknown attribute name '${name}' encountered. \
