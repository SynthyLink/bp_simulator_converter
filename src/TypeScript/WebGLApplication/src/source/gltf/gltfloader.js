...
const gltf_loader_ts_1 = require("gltf-loader-ts");
const gltf_1 = require("gltf-loader-ts/lib/gltf");
const gltfhelper_1 = require("./gltfhelper");
const gltfmesh_1 = require("./gltfmesh");
const gltfpbrmaterial_1 = require("./gltfpbrmaterial");
...
const scenenode_1 = require("../scene/scenenode");
...
class GLTFLoader {
    ...
    constructor(context) {
        ...
        const vert = new shader_1.Shader(this._context, gl.VERTEX_SHADER, 'gltf_default.vert');
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        vert.initialize(require('./shaders/gltf_default.vert'));
        const frag = new shader_1.Shader(this._context, gl.FRAGMENT_SHADER, 'gltf_default.frag');
        // eslint-disable-next-line @typescript-eslint/no-var-requires
    ...
    async loadTextures(asset) {
        const gl = this._context.gl;
        this._uriTextureMap = new Map();
        ...
        for (const textureInfo of textures) {
            if (textureInfo.source === undefined) {
                (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `The GLTF model does not specify a texture source. Possibly it uses an unsupported extension.`);
                textureId++;
                continue;
            }
            ...
            let data = await asset.imageData.get(imageId);
            /**
             ...
             */
            if (!(0, auxiliaries_1.isPowerOfTwo)(data.width) || !(0, auxiliaries_1.isPowerOfTwo)(data.height)) {
                // Scale up the texture to the next highest power of two dimensions.
                const canvas = document.createElement('canvas');
                canvas.width = (0, auxiliaries_1.upperPowerOfTwo)(data.width);
                canvas.height = (0, auxiliaries_1.upperPowerOfTwo)(data.height);
                const ctx = canvas.getContext('2d');
                if (ctx === undefined) {
                    (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Error, 'Failed to create context while trying to resize non power of two texture');
                }
                ctx.drawImage(data, 0, 0, canvas.width, canvas.height);
                data = canvas;
            }
            const texture = new texture2d_1.Texture2D(this._context, name);
            ...
            const uri = image.uri ? image.uri : identifier;
            this._uriTextureMap.set(uri, texture);
            textureId++;
        }
    }
    getTexture(index) {
        ...
        if (texture === undefined) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `Texture with index ${index} could not be located.`);
            return;
        }
        return texture;
    }
    async loadMaterials(asset) {
        ...
        for (const materialInfo of materials) {
            ...
            const material = new gltfpbrmaterial_1.GLTFPbrMaterial(this._context, materialInfo.name);
            ...
            if (pbrInfo === undefined) {
                (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, 'Model contains a material without PBR information');
            }
    ...
    inferBufferUsage(asset, bufferViewId) {
        const gl = this._context.gl;
        const meshes = asset.gltf.meshes;
        const accessors = asset.gltf.accessors;
        if (meshes === undefined || accessors === undefined) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Error, `Asset does not include any meshes or accessors`);
    ...
    async loadBuffers(asset) {
        const gl = this._context.gl;
        const bufferViews = asset.gltf.bufferViews;
        const accessors = asset.gltf.accessors;
        if (!bufferViews) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, 'The asset does not include any buffer view information.');
            return;
        }
        if (!accessors) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, 'The asset does not include any accessor information.');
            return;
        }
        /**
        ...
        for (const bufferViewInfo of bufferViews) {
            ...
            const buffer = new buffer_1.Buffer(this._context, identifier);
    ...
    async loadMeshes(asset) {
        this._meshes = new Array();
        this._nameMeshMap = new Map();
        const meshes = asset.gltf.meshes;
        if (meshes === undefined) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, 'The asset does not contain any mesh information');
            return this._meshes;
        }
        let primitiveId = 0;
        for (const meshInfo of meshes) {
            const mesh = new gltfmesh_1.GLTFMesh(meshInfo.name);
            ...
            if (meshInfo.name) {
                this._nameMeshMap.set(meshInfo.name, mesh);
            }
            this._meshes.push(mesh);
        }
        return this._meshes;
    }
    async loadPrimitive(asset, primitiveInfo, id) {
        const gl = this._context.gl;
        const accessors = asset.gltf.accessors;
        const bufferViews = asset.gltf.bufferViews;
        if (accessors === undefined) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Error, 'GLTF asset does not have any accessors.');
            return;
        }
        if (bufferViews === undefined) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Error, 'GLTF asset does not have any buffer views.');
            return;
        }
        ...
        const drawMode = gltfhelper_1.GLTFHelper.modeToEnum(gl, modeNumber);
        ...
        if (primitiveInfo.material !== undefined) {
            ...
            if (fetchedMaterial !== undefined) {
                ...
                (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `Material ${materialIdentifier} could not be found.`);
            }
        }
        let geometryFlags = 0;
        for (const semantic in primitiveInfo.attributes) {
            // init buffer/attribute binding for attribute
            const attributeIndex = gltfhelper_1.GLTFHelper.nameToAttributeIndex(semantic);
            if (semantic === 'NORMAL') {
            ...
            const binding = new gltfprimitive_1.VertexBinding();
        ...
        if (primitiveInfo.indices !== undefined) {
            ...
            if (!bufferViewIndex === undefined) {
                (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Error, 'Accessor does not reference a BufferView.');
            }
            const bufferViewIdentifier = this._sceneName + '_bufferView_' + bufferViewIndex;
            const buffer = this._resourceManager.get(bufferViewIdentifier);
        ...
        const primitive = new gltfprimitive_1.GLTFPrimitive(this._context, bindings, indexBinding, drawMode, material, geometryFlags);
        primitive.initialize();
        this._resourceManager.add(primitive, [identifier]);
        return primitive;
    }
    async generateScenes(asset) {
        const nodes = asset.gltf.nodes;
        const scenes = asset.gltf.scenes;
        if (!nodes || !scenes) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, 'The asset does not contain any nodes or scene information.');
            return;
        }
        ...
        for (const node of nodes) {
            ...
            const sceneNode = new scenenode_1.SceneNode(name);
            idToNode.set(nodeId, sceneNode);
            if (node.matrix) {
                const matrix = gl_matrix_1.mat4.fromValues.apply(undefined, node.matrix);
                const transformComponent = new scene_1.TransformComponent(matrix);
                sceneNode.addComponent(transformComponent);
            }
            else if (node.translation || node.rotation || node.scale) {
                let rotation = gl_matrix_1.quat.create();
                if (node.rotation) {
                ...
                let scale = gl_matrix_1.vec3.fromValues(1, 1, 1);
                if (node.scale) {
                    scale = gl_matrix_1.vec3.fromValues.apply(undefined, node.scale);
                }
                const matrix = gl_matrix_1.mat4.fromRotationTranslationScale(gl_matrix_1.mat4.create(), rotation, translation, scale);
                const transformComponent = new scene_1.TransformComponent(matrix);
                sceneNode.addComponent(transformComponent);
            }
            if (node.mesh !== undefined) {
                const mesh = this._meshes[node.mesh];
                for (const primitive of mesh.primitives) {
                    const geometryComponent = new scene_1.GeometryComponent();
            ...
            if (node.skin || node.weights) {
                (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Info, 'Imported GLTF assets uses skins, which are not supported yet.');
            }
            nodeId++;
        }
        /**
        ...
        for (const node of nodes) {
            ...
            (0, auxiliaries_1.assert)(sceneNode !== undefined, 'Scene node could not be found.');
            if (node.children) {
                for (const childId of node.children) {
                    const childNode = idToNode.get(childId);
                    if (!childNode) {
                        (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Error, 'Model references a node that does not exist.');
                    }
        ...
        for (const scene of scenes) {
            ...
            if (scene.nodes) {
                for (const nodeId of scene.nodes) {
                    ...
                    if (!node) {
                        (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Error, 'Scene references a node that does not exist.');
                    }
                    sceneNode.addNode(node);
                }
            }
            else {
                (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `Scene ${name} does not contain any nodes.`);
            }
    ...
    async loadAsset(uri) {
        const loader = new gltf_loader_ts_1.GltfLoader();
        ...
        await this.loadTextures(asset)
            ...
            .then(() => this.generateScenes(asset));
    }
    uninitialize() {
        this._scenes = [];
     ...
     */
    getMeshByName(name) {
        return this._nameMeshMap.get(name);
    }
    /**
    ...
    get defaultScene() {
        if (this._defaultScene !== undefined) {
            ...
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, 'Default scene was requested, but none is available.');
