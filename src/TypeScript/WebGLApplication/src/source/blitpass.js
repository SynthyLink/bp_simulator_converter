 ...
 * ```
 */
class BlitPass extends initializable_1.Initializable {
    /**
    ...
    functionBlit() {
        ...
        if (this._drawBuffer >= gl.COLOR_ATTACHMENT1 &&
    ...
    programBlit() {
        ...
        (0, auxiliaries_1.assert)(this._ndcTriangle && this._ndcTriangle.initialized, `expected an initialized ndc triangle`);
        const gl = this._context.gl;
        const srcBounds = this._srcBounds ? this._srcBounds :
            gl_matrix_1.vec4.fromValues(0, 0, this._framebuffer.width, this._framebuffer.height);
        const dstBounds = this._dstBounds ? this._dstBounds :
            gl_matrix_1.vec4.fromValues(0, 0, this._target.width, this._target.height);
        const srcBoundsNormalized = gl_matrix_1.vec4.div((0, gl_matrix_extensions_1.v4)(), srcBounds, [this._framebuffer.width, this._framebuffer.height, this._framebuffer.width, this._framebuffer.height]);
        const dstBoundsNormalized = gl_matrix_1.vec4.div((0, gl_matrix_extensions_1.v4)(), dstBounds, [this._target.width, this._target.height, this._target.width, this._target.height]);
        ...
        const texture = this._framebuffer.texture(this._readBuffer);
    ...
    createProgram() {
        ...
        const gl = this._context.gl;
        const vert = new shader_1.Shader(this._context, gl.VERTEX_SHADER, 'blit.vert (blit)');
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        vert.initialize(require('./shaders/blit.vert'));
        const frag = new shader_1.Shader(this._context, gl.FRAGMENT_SHADER, 'blit.frag (blit)');
        // eslint-disable-next-line @typescript-eslint/no-var-requires
    ...
    frame() {
        (0, auxiliaries_1.logIf)(!this._target || !this._target.valid, auxiliaries_1.LogLevel.Warning, `valid target expected, given ${this._target}`);
        (0, auxiliaries_1.logIf)(!this._framebuffer || !this._framebuffer.valid, auxiliaries_1.LogLevel.Warning, `valid framebuffer for blitting from expected, given ${this._framebuffer}`);
        const gl = this._context.gl;
     ...
     */
    set enforceProgramBlit(enforce) {
