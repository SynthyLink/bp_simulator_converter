...
class Icosahedron {
    ...
    split(a, b, vertices, cache) {
        ...
        const pos = gl_matrix_1.vec3.add((0, gl_matrix_extensions_1.v3)(), vertices[a], vertices[b]);
    ...
    generateTextureCoordinates() {
        ...
        for (const pos of this._vertices) {
            const normal = gl_matrix_1.vec3.normalize((0, gl_matrix_extensions_1.v3)(), pos);
            const uv = gl_matrix_1.vec2.fromValues(ofs - (Math.atan2(normal[2], normal[0]) / (2.0 * Math.PI)), Math.asin(normal[1]) / Math.PI + ofs);
