 ...
 * ```
 */
class Framebuffer extends object_1.AbstractObject {
    /**
     * Default framebuffer, e.g., used for unbind.
     */
    static DEFAULT_FRAMEBUFFER = undefined;
    /**
     * Access to all attached renderbuffer objects.
     */
    _buffersByAttachment = new Map();
    /**
     * Access to all attached texture objects.
     */
    _texturesByAttachment = new Map();
    /**
     ...
     */
    _colorClearQueue = new Array();
    /** @see {@link drawBuffers} */
    _drawBuffers = new Array();
     ...
     */
    static statusString(context, status) {
        const gl = context.gl;
        switch (status) {
            ...
            default:
                (0, auxiliaries_1.assert)(false, `expected known framebuffer status, given ${status}`);
                return '';
        }
    }
    /**
    ...
    create(attachments) {
        ...
        this._buffersByAttachment.forEach((buffer, attachment) => {
            gl.framebufferRenderbuffer(gl.FRAMEBUFFER, attachment, gl.RENDERBUFFER, buffer.object);
        });
        this._texturesByAttachment.forEach((texture, attachment) => {
        ...
        (0, auxiliaries_1.logIf)(!this._valid, auxiliaries_1.LogLevel.Warning, Framebuffer.statusString(this.context, status));
     ...
     */
    delete() {
        (0, auxiliaries_1.assert)(this._object instanceof WebGLFramebuffer, `expected WebGLFramebuffer object`);
    ...
    es2Clear(mask, bind = true, unbind = true) {
        const gl = this.context.gl;
        const clearDepth = (0, auxiliaries_1.bitInBitfield)(mask, gl.DEPTH_BUFFER_BIT);
        const clearStencil = (0, auxiliaries_1.bitInBitfield)(mask, gl.STENCIL_BUFFER_BIT);
        const clearColor = (0, auxiliaries_1.bitInBitfield)(mask, gl.COLOR_BUFFER_BIT);
     ...
     */
    @initializable_1.Initializable.assert_initialized()
    es3Clear(mask, bind = true, unbind = true, colorClearQueue) {
        const gl = this.context.gl;
        const clearDepth = (0, auxiliaries_1.bitInBitfield)(mask, gl.DEPTH_BUFFER_BIT);
        const clearStencil = (0, auxiliaries_1.bitInBitfield)(mask, gl.STENCIL_BUFFER_BIT);
        const clearColor = (0, auxiliaries_1.bitInBitfield)(mask, gl.COLOR_BUFFER_BIT);
     ...
     */
    @initializable_1.Initializable.assert_initialized()
    unbind(target = this.context.gl.FRAMEBUFFER) {
     ...
     */
    @initializable_1.Initializable.assert_initialized()
    clearColor(color, drawBuffer) {
        (0, auxiliaries_1.assert)(drawBuffer === undefined || drawBuffer === 0 || this.context.isWebGL2 ||
            this.context.supportsDrawBuffers, `WebGL2 context expected for clearing multiple color attachments.`);
        const alphaIssue = color[3] < 1.0 && !this.context.alpha;
        (0, auxiliaries_1.logIf)(alphaIssue, auxiliaries_1.LogLevel.Warning, `context has alpha disabled, clear color alpha is ignored`);
        const color2 = [color[0], color[1], color[2], alphaIssue ? 1.0 : color[3]];
    ...
    resize(width, height, bind = true, unbind = true) {
        this._buffersByAttachment.forEach((buffer) => {
            buffer.resize(width, height, bind, unbind);
        });
        this._texturesByAttachment.forEach((texture) => {
     ...
     */
    set drawBuffers(attachments) {
        this.assertInitialized();
        const gl2facade = this.context.gl2facade;
        for (const attachment of attachments) {
            (0, auxiliaries_1.assert)(this.hasAttachment(attachment), `valid attachment expected for draw buffer, given ${attachment}`);
            (0, auxiliaries_1.assert)(attachment >= gl2facade.COLOR_ATTACHMENT_MIN && attachment <= gl2facade.COLOR_ATTACHMENT_MAX, `color attachment expected for draw buffer, given ${attachment}`);
