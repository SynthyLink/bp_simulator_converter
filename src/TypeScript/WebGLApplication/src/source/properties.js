 ...
 */
function complementProperty(instance, schema) {
    ...
    for (const key of props) {
        ...
        (0, auxiliaries_1.assert)((hasProperties && isObject) || (!hasProperties && !isObject), `expected property '${key}' to be of type 'object', given '${propertySchema['type']}'`);
 ...
 * @param schema - Schema used for validation.
 */
function complementArray(instance, schema) {
...
(function (properties) {
    ...
    function validate(instance, schema, references) {
        ...
        (0, auxiliaries_1.logIf)(!result.valid, auxiliaries_1.LogLevel.Warning, `schema expected to be valid:\n${result.toString()}`);
        return result.valid;
    }
    properties.validate = validate;
    /**
    ...
    function complement(instance, schema) {
        ...
        (0, auxiliaries_1.assert)((schema.hasOwnProperty('properties') && schema['type'] === 'object') ||
    ...
    properties.complement = complement;
    /**
    ...
    function compare(objectL, objectR, tracker, path = '') {
        ...
        // eslint-disable-next-line @typescript-eslint/ban-types
        (0, auxiliaries_1.assert)(!track || tracker.hasOwnProperty('any'), `expected allocation lookup object to have 'any' key`);
        ...
        let types = [typeof objectL, typeof objectR];
        const isArray = [objectL instanceof Array, objectR instanceof Array];
        ...
        const names = Array.from(new Set(Array().concat(Object.getOwnPropertyNames(objectL), Object.getOwnPropertyNames(objectR))).values());
        /* Deep compare if both operands are either object or array. */
        for (const name of names) {
            if (name === 'length') {
                continue;
            }
            const values = [objectL[name], objectR[name]];
