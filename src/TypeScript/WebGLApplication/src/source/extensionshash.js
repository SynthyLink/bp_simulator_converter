...
exports.ExtensionsHash = void 0;
const auxiliaries_1 = require("./auxiliaries");
const context_1 = require("./context");
const extensions_1 = require("./extensions");
/* spellchecker: enable */
class ExtensionsHash {
    static WEBGL_BACKENDS = ['webgl1', 'webgl2'];
    static WEBGL_EXTENSIONS_BY_BACKEND = new Map([
     ...
     */
    static EXTENSIONS_BY_VERSION = new Map([[
            0b000, [
     ...
     * existing hashes.
     */
    static LATEST_VERSION = 0b000;
    static BASE64_ALPHABET = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-+';
    /**
    ...
    static encode64(bitfield) {
        (0, auxiliaries_1.assert)(bitfield >= 0b000000 && bitfield <= 0b111111, `expected bitfield in range [ 0b000000, 0b111111 ], given ${bitfield}`);
    ...
    static decode64(base64) {
        (0, auxiliaries_1.assert)(base64.length === 1, `expected single base64 character, given '${base64}'`);
        const bitfield = ExtensionsHash.BASE64_ALPHABET.indexOf(base64);
        (0, auxiliaries_1.assert)(bitfield > -1, `unknown base64 character, given '${base64}'`);
        return bitfield;
    }
    /**
     ...
     */
    static encode(backend, supported) {
        const version = ExtensionsHash.LATEST_VERSION; // should always be set to the latest version
        const extensions = ExtensionsHash.EXTENSIONS_BY_VERSION.get(version);
        const backendIndex = ExtensionsHash.WEBGL_BACKENDS.indexOf(backend);
        (0, auxiliaries_1.assert)(backendIndex > -1, `expected valid backend ${ExtensionsHash.WEBGL_BACKENDS}, given ${backend} `);
        let hash = ExtensionsHash.encode64((version << 3) | (backendIndex + 1));
    ...
    static decode(hash) {
        ...
        (0, auxiliaries_1.assert)(backendIndex < ExtensionsHash.WEBGL_BACKENDS.length, `expected valid backend index, given ${backendIndex}`);
        const backend = ExtensionsHash.WEBGL_BACKENDS[backendIndex];
        (0, auxiliaries_1.assert)(ExtensionsHash.EXTENSIONS_BY_VERSION.has(version), `expected valid hash version, given ${version}`);
        const extensions = ExtensionsHash.EXTENSIONS_BY_VERSION.get(version);
        const expectedHashLength = Math.ceil(extensions.length / 6) + 1;
        (0, auxiliaries_1.assert)(hash.length === expectedHashLength, `expected hash of version ${version} to have a length of ${expectedHashLength}, given ${hash}`);
        const strived = new Array();
     ...
     */
    static complement(backend, extensions) {
        const backendIndex = ExtensionsHash.WEBGL_BACKENDS.indexOf(backend);
        (0, auxiliaries_1.assert)(backendIndex > -1, `expected valid backend ${ExtensionsHash.WEBGL_BACKENDS}, given ${backend} `);
        const webglExtensions = ExtensionsHash.WEBGL_EXTENSIONS_BY_BACKEND.get(backendIndex);
        return webglExtensions.filter((ext) => extensions.indexOf(ext) < 0);
    }
}
exports.ExtensionsHash = ExtensionsHash;
//# sourceMappingURL=extensionshash.js.map