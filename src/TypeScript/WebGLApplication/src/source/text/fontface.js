...
const fontfaceloader_1 = require("./fontfaceloader");
...
class FontFace {
    ...
    _glyphTextureExtent = [0.0, 0.0];
    /** @see {@link glyphTexturePadding} */
    _glyphTexturePadding = [0.0, 0.0, 0.0, 0.0];
    /** @see {@link glyphTexturePadding} */
    _glyphTexture;
    /**
     ...
     */
    static fromFile(url, context, headless = false, identifier) {
        const transform = (data) => {
            const font = new FontFace(context, identifier);
            return fontfaceloader_1.FontFaceLoader.process(font, data, url, undefined, headless)
                .then((fontFace) => fontFace)
                .catch((reason) => Promise.reject(`processing font face data failed: ${reason}`));
        };
        return (0, fetch_1.fetchAsync)(url, 'text').then(transform);
    }
    static fromFiles(fontFileUrl, pageFileUrlsByPageID, context, headless = false, identifier) {
        const transform = (data) => {
            const font = new FontFace(context, identifier);
            return fontfaceloader_1.FontFaceLoader.process(font, data, fontFileUrl, pageFileUrlsByPageID, headless)
                .then((fontFace) => fontFace)
                .catch((reason) => Promise.reject(`processing font face data failed: ${reason}`));
        };
        return (0, fetch_1.fetchAsync)(fontFileUrl, 'text').then(transform);
    }
    /**
    ...
    constructor(context, identifier) {
        ...
        const internalFormat = wizard_1.Wizard.queryInternalTextureFormat(context, gl.RGBA, wizard_1.Wizard.Precision.byte);
     ...
     */
    addGlyph(glyph) {
        (0, auxiliaries_1.assert)(!(this._glyphs.get(glyph.index)), 'expected glyph to not already exist');
     ...
     */
    setKerning(index, subsequentIndex, kerning) {
        const glyph = this._glyphs.get(index);
        if (!glyph || !this.hasGlyph(subsequentIndex)) {
            (0, auxiliaries_1.assert)(false, `expected glyph or glyph of subsequent index to exist, \
     ...
     */
    set size(size) {
        (0, auxiliaries_1.assert)(size > 0.0, `expected size to be greater than 0.0, given ${size}`);
        this._size = size;
    }
    get size() {
        return this._size;
     ...
     */
    set base(base) {
        (0, auxiliaries_1.assert)(base > 0.0, `expected base to be greater than 0.0, given ${base}`);
        this._base = base;
    }
    get base() {
        return this._base;
     ...
     */
    set ascent(ascent) {
        (0, auxiliaries_1.assert)(ascent > 0.0, `expected ascent to be greater than 0.0, given ${ascent}`);
        this._ascent = ascent;
    }
    get ascent() {
        return this._ascent;
    ...
    set lineHeight(lineHeight) {
        (0, auxiliaries_1.assert)(this.size > 0, `expected size to be greater than zero to derive line gap from line height`);
     ...
     */
    set glyphTextureExtent(extent) {
        (0, auxiliaries_1.assert)(extent[0] > 0, `expected extent.x to be greater than 0.0, given ${extent[0]}`);
        (0, auxiliaries_1.assert)(extent[1] > 0, `expected extent.y to be greater than 0.0, given ${extent[1]}`);
     ...
     * px.
     */
    set glyphTexturePadding(padding) {
        (0, auxiliaries_1.assert)(padding[0] >= 0.0, `expected padding[0] to be greater than 0.0, given ${padding[0]}`);
        (0, auxiliaries_1.assert)(padding[1] >= 0.0, `expected padding[1] to be greater than 0.0, given ${padding[1]}`);
        (0, auxiliaries_1.assert)(padding[2] >= 0.0, `expected padding[2] to be greater than 0.0, given ${padding[2]}`);
        (0, auxiliaries_1.assert)(padding[3] >= 0.0, `expected padding[3] to be greater than 0.0, given ${padding[3]}`);
        this._glyphTexturePadding = padding;
    }
    get glyphTexturePadding() {
