...
class FontFaceLoader {
     ...
     */
    static processInfo(stream, fontFace) {
        const pairs = new Map();
        const success = this.readKeyValuePairs(stream, ['size', 'padding'], pairs);
        if (!success) {
            return false;
        }
        ...
        if (values.length !== 4) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `expected 4 values for padding, given ${values} (${values.length})`);
            return false;
        }
        const padding = [
     ...
     */
    static processCommon(stream, fontFace) {
        const pairs = new Map();
        const success = this.readKeyValuePairs(stream, ['lineHeight', 'base', 'scaleW', 'scaleH'], pairs);
        if (!success) {
            return false;
        }
     ...
     */
    static processPage(stream, fontFace, url) {
        const pairs = new Map();
        const success = this.readKeyValuePairs(stream, ['file'], pairs);
        if (!success) {
            return undefined;
        }
        const path = (0, auxiliaries_1.dirname)(url);
     ...
     * @returns - Promise for handling image load status.
     */
    static processPages(fontFace, pageFileUrlsByPageID) {
        (0, auxiliaries_1.logIf)(pageFileUrlsByPageID.size !== 1, auxiliaries_1.LogLevel.Warning, 'glyph atlas supports a single page only (impl. for multiple pages pending)');
        (0, auxiliaries_1.assert)(pageFileUrlsByPageID.has(0), 'page with page ID 0 expected for glyph atlas (impl. for multiple pages pending)');
        const page = 0;
     ...
     * @param fontFace - The font face in which the loaded glyph texture is stored.
     */
    static processChar(stream, fontFace) {
        const pairs = new Map();
        const success = this.readKeyValuePairs(stream, ['id', 'x', 'y', 'width', 'height', 'xoffset', 'yoffset', 'xadvance'], pairs);
        if (!success) {
            return false;
        }
        const index = parseInt(pairs.get('id'), 10);
        (0, auxiliaries_1.logIf)(index <= 0.0, auxiliaries_1.LogLevel.Warning, `expected glyph index to be greater than 0, given ${index}`);
     ...
     */
    static processKerning(stream, fontFace) {
        const pairs = new Map();
        const success = this.readKeyValuePairs(stream, ['first', 'second', 'amount'], pairs);
        if (!success) {
            return false;
        }
        const first = parseInt(pairs.get('first'), 10);
        if (first <= 0.0) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `expected kerning's first to be greater than 0, given ${first}`);
            return false;
        }
        const second = parseInt(pairs.get('second'), 10);
        if (second <= 0.0) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `expected kerning's second to be greater than 0, given ${second}`);
            return false;
        }
        const kerning = parseFloat(pairs.get('amount'));
     ...
     * @returns - success
     */
    static readKeyValuePairs(stream, mandatoryKeys, resultPairs) {
        let key;
        let value;
        ...
        if (!valid) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `Not all required keys are provided! Mandatory keys: ${mandatoryKeys}`);
        }
        return valid;
    }
    /**
     ...
     */
    static findAscentAndDescentIfNoneProvided(fontFace, size) {
        ...
        fontFace.descent = fontFace.ascent - size;
        (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Debug, `ascent not specified, derived ${fontFace.ascent} from maximum y-offset`);
    }
    /**
     ...
     */
    static process(fontFace, data, fontFileUrl, pageFileUrlsByPageID, headless = false) {
        const lines = data.split('\n');
        const promises = new Array();
        let status = true;
        if (!headless && pageFileUrlsByPageID) {
            const promise = this.processPages(fontFace, pageFileUrlsByPageID);
        ...
        for (const line of lines) {
            ...
            switch (identifier) {
                ...
                case 'page':
                    ...
                    const promise = this.processPage(attributes, fontFace, fontFileUrl);
                    if (promise !== undefined) {
                        promises.push(promise);
                    }
                    break;
        ...
        if (fontFace.size <= 0.0) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `expected fontFace.size to be greater than 0, given ${fontFace.size}`);
        }
