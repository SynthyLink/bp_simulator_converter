...
class Typesetter {
     ...
     */
    static writeVertex(fontFace, pen, glyph, vertices, index) {
        ...
        const lowerLeft = vertices.uvLowerLeft(index);
        gl_matrix_1.vec2.copy(lowerLeft, glyph.subTextureOrigin);
        const upperRight = vertices.uvUpperRight(index);
     ...
     */
    static advances(label, text) {
        (0, auxiliaries_1.assert)(label.fontFace !== undefined, `expected a font face for label in order to resolve advances`);
        if (text === undefined) {
     ...
     */
    static kernings(label, text) {
        (0, auxiliaries_1.assert)(label.fontFace !== undefined, `expected a font face for label in order to resolve kernings`);
        if (text === undefined) {
     ...
     * @returns -
     */
    static fragmentWidths(fragments, advances, kernings) {
        const widths = new Float32Array(fragments.length);
        for (let i = 0; i < fragments.length; ++i) {
            const fragment = fragments[i];
            widths[i] = advances.subarray(fragment[0], fragment[1]).reduce((width, advance, index) => width + advance + (index < fragment[1] ? kernings[index + fragment[0]] : 0.0), 0.0);
        }
        return widths;
    }
    /**
     ...
     */
    static elideThresholds(label, ellipsisWidth) {
        switch (label.elide) {
     ...
     */
    static elideFragments(threshold, labelFragments, labelFragmentWidths, labelAdvances, labelKernings, reverse) {
        const fragments = new Array();
        const fragmentWidths = new Array();
        let width = 0.0;
        let lastLabelFragmentWidth = 0.0;
        // tslint:disable-next-line:prefer-for-of
        for (let i0 = reverse ? labelFragments.length - 1 : 0; reverse ? i0 > -1 : i0 < labelFragments.length; reverse ? --i0 : ++i0) {
            ...
            for (let i1 = reverse ? fragment[1] - 1 : fragment[0]; reverse ? i1 >= fragment[0] : i1 < fragment[1]; reverse ? --i1 : ++i1) {
     ...
     */
    static transformVertices(transform, vertices, begin, end) {
        if (vertices === undefined || gl_matrix_1.mat4.equals(transform, gl_matrix_1.mat4.create())) {
            return;
        }
        for (let index = begin; index < end; ++index) {
            const origin = vertices.origin(index);
            const ll = (0, gl_matrix_extensions_1.v3)(); // Lower Left
            gl_matrix_1.vec3.transformMat4(ll, origin, transform);
            const lr = (0, gl_matrix_extensions_1.v3)(); // Lower Right
            gl_matrix_1.vec3.transformMat4(lr, gl_matrix_1.vec3.add(lr, origin, vertices.tangent(index)), transform);
            const ul = (0, gl_matrix_extensions_1.v3)(); // Upper Left
            gl_matrix_1.vec3.transformMat4(ul, gl_matrix_1.vec3.add(ul, origin, vertices.up(index)), transform);
     ...
     */
    static updateRectangleMinMax(currentRectangle, newRectangle) {
        (0, auxiliaries_1.assert)(currentRectangle.length === 6 && newRectangle.length === 6, `expected the rectangles to have 6 values!`);
        let i = 0;
        for (; i < 3; i++) {
     ...
     */
    static minMax(currentMin, currentMax, values) {
     ...
     * @param end - Vertex index to stop alignment at.
     */
    static getMinMaxVertices(vertices, begin, end) {
     ...
     * @param end - Vertex index to stop alignment at.
     */
    static transformAlignment(width, alignment, vertices, begin, end) {
     ...
     */
    static transform(label, vertices, lines) {
        ...
        const ll = gl_matrix_1.vec4.transformMat4(gl_matrix_1.vec4.create(), gl_matrix_1.vec4.fromValues(0, 0, 0, 1), label.staticTransform);
        const lr = gl_matrix_1.vec4.transformMat4(gl_matrix_1.vec4.create(), gl_matrix_1.vec4.fromValues(width, 0, 0, 1), label.staticTransform);
        const ul = gl_matrix_1.vec4.transformMat4(gl_matrix_1.vec4.create(), gl_matrix_1.vec4.fromValues(0, height, 0, 1), label.staticTransform);
        const extent = gl_matrix_1.vec2.fromValues(gl_matrix_1.vec4.distance(lr, ll), gl_matrix_1.vec4.distance(ul, ll));
     ...
     */
    static typeset(label, vertices) {
        if (label.length === 0) {
            return 0;
        }
        (0, auxiliaries_1.assert)(label.fontFace !== undefined, `expected a font face for label before typesetting`);
        const fontFace = label.fontFace;
        ...
        const glyphs = (index) => index < label.length ? fontFace.glyph(label.charCodeAt(index)) :
        ...
        const pen = gl_matrix_1.vec2.fromValues(0.0, -Typesetter.lineAnchorOffset(label));
        const lines = new Array();
        let vertexIndex = 0;
        const elide = label.elide !== label_1.Label.Elide.None;
        ...
        const advance = (fragments, fragmentWidths, threshold = NaN, advances = labelAdvances, kernings = labelKernings, offset = 0) => {
        ...
        if (elide && fullApproximatedWidth > label.lineWidth) {
            ...
            const ellipsisFragments = [[label.length, label.length + label.ellipsis.length, Typesetter.FragmentType.Word]];
            ...
            const ellipsisFragmentWidths = Typesetter.fragmentWidths([[0, label.ellipsis.length, Typesetter.FragmentType.Word]], ellipsisAdvances, ellipsisKernings);
            ...
            const [leftFragments, leftFragmentWidths, leftWidth] = Typesetter.elideFragments(thresholds[0], labelFragments, labelFragmentWidths, labelAdvances, labelKernings, false);
            ...
            const [rightFragments, rightFragmentWidths] = Typesetter.elideFragments(thresholds[1], labelFragments, labelFragmentWidths, labelAdvances, labelKernings, true);
            ...
            (0, auxiliaries_1.assert)(lines.length < 4, `expected at most 3-line-yield from advance, given ${lines}`);
...
})(Typesetter || (exports.Typesetter = Typesetter = {}));
