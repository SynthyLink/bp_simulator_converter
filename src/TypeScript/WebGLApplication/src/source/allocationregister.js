...
exports.AllocationRegister = void 0;
...
class AllocationRegister {
     ...
     */
    _bytesByIdentifier = new Map();
    /** @see {@link bytes} */
    _bytes = 0;
    _bytesSubject = new rxjs_1.ReplaySubject(1);
    /**
    ...
    assertIdentifier(identifier) {
        (0, auxiliaries_1.assert)(this._bytesByIdentifier.has(identifier), `allocation identifier unknown`);
    }
    /**
     ...
     */
    createUniqueIdentifier(identifier) {
     ...
     */
    deleteUniqueIdentifier(identifier) {
        (0, auxiliaries_1.assert)(this._bytesByIdentifier.has(identifier), `identifier expected to be known for allocation registration`);
    ...
    allocate(identifier, allocate) {
        ...
        (0, auxiliaries_1.assert)(allocate >= 0, `positive number of bytes expected for allocation, given ${allocate}`);
     ...
     */
    deallocate(identifier, deallocate) {
        this.assertIdentifier(identifier);
        const bytes = this._bytesByIdentifier.get(identifier);
        (0, auxiliaries_1.assert)(deallocate >= 0, `positive number of bytes expected for deallocation, given ${deallocate}`);
        (0, auxiliaries_1.assert)(deallocate <= bytes, `deallocation cannot exceed previous allocations of ${bytes}, given ${deallocate}`);
     ...
     */
    reallocate(identifier, reallocate) {
        this.assertIdentifier(identifier);
        (0, auxiliaries_1.assert)(reallocate >= 0, `positive number of bytes expected for reallocation, given ${reallocate}`);
    ...
    toString() {
        ...
        this._bytesByIdentifier.forEach((bytes, identifier) => {
            output.push(`${identifier}: ${(0, auxiliaries_1.prettyPrintBytes)(bytes)}`);
        });
        return output.join(', ');
    }
    /**
     ...
     */
    bytesToString(identifier) {
        return (0, auxiliaries_1.prettyPrintBytes)(this.allocated(identifier));
    }
    /**
...
//# sourceMappingURL=allocationregister.js.map