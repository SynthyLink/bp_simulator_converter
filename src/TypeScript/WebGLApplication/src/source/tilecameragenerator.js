...
exports.TileCameraGenerator = void 0;
...
class TileCameraGenerator {
    ...
    _tileSize = [0, 0];
    /** @see {@link padding} */
    _padding = gl_matrix_1.vec4.fromValues(0, 0, 0, 0);
    /** @see {@link tile} */
    _tile = -1;
    /** @see {@link eye} */
    _camera;
    /** @see {@link algorithm} */
    _algorithm = TileCameraGenerator.Algorithm.ScanLine;
    _valid;
    _offset = [0, 0];
    _indices;
    /**
     ...
     */
    static hilbertIndices(indices, numX, numY, x, y, xi, xj, yi, yj, depth, hilbertIndex) {
        if (depth > 0) {
     ...
     */
    static generateHilbertIndices(indices, numX, numY) {
        (0, auxiliaries_1.assert)(indices.length === 2 * numX * numY, `expected interleaved indices-array of length ${2 * numX * numY}, given ${indices.length}`);
        const tableSize = Math.max(numX, numY);
        const recursionDepth = Math.ceil(Math.log2(tableSize));
        const uPow2 = (0, auxiliaries_1.upperPowerOfTwo)(tableSize);
     ...
     */
    static generateScanLineIndices(indices, numX, numY) {
        (0, auxiliaries_1.assert)(indices.length === 2 * numX * numY, `expected interleaved indices-array of length ${2 * numX * numY}, given ${indices.length}`);
     ...
     */
    static generateZCurveIndices(indices, numX, numY) {
        (0, auxiliaries_1.assert)(indices.length === 2 * numX * numY, `expected interleaved indices-array of length ${2 * numX * numY}, given ${indices.length}`);
        const tableSize = Math.max(numX, numY);
     ...
     */
    sourceCameraChanged() {
        (0, auxiliaries_1.assert)(this._sourceCamera !== undefined, `expected the unput/source camera to be defined`);
    ...
    update() {
        ...
        if (this.numTiles <= this.tile || 0 > this.tile) {
            (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `index ${this.tile} is out of bounds ${this.numTiles}, returning first tile`);
            return this._offset;
        }
        (0, auxiliaries_1.assert)(this._sourceViewport !== undefined && this._sourceCamera !== undefined, `expected source camera and source viewport to be defined before updating`);
        this._valid = true;
        ...
        const scaleVec = gl_matrix_1.vec3.fromValues(viewport[0] / paddedTileSize[0], viewport[1] / paddedTileSize[1], 1);
        // Create the translation vector which shifts the padded tile center NDC into the origin.
        const translationVec = gl_matrix_1.vec3.fromValues(-paddedTileCenterNDC[0], -paddedTileCenterNDC[1], 0);
        // Combine the translation ans scale into the matrix.
        const tileNDCCorrectionMatrix = gl_matrix_1.mat4.scale((0, gl_matrix_extensions_1.m4)(), gl_matrix_1.mat4.identity((0, gl_matrix_extensions_1.m4)()), scaleVec);
        const translateMatrix = gl_matrix_1.mat4.translate((0, gl_matrix_extensions_1.m4)(), tileNDCCorrectionMatrix, translationVec);
     ...
     */
    get numXTiles() {
        (0, auxiliaries_1.assert)(this._sourceViewport !== undefined, `expected the source viewport to be defined`);
...
})(TileCameraGenerator || (exports.TileCameraGenerator = TileCameraGenerator = {}));
//# sourceMappingURL=tilecameragenerator.js.map