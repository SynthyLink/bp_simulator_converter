 ...
 */
class Buffer extends object_1.AbstractObject {
    /**
     * Default buffer, e.g., used for unbind.
     */
    static DEFAULT_BUFFER = undefined;
    /** @see {@link target} */
    _target = Buffer.DEFAULT_BUFFER;
    /**
    ...
    create(target) {
        ...
        if (this._valid) {
            (0, auxiliaries_1.assert)(target === gl.ARRAY_BUFFER || target === gl.ELEMENT_ARRAY_BUFFER, `either ARRAY_BUFFER or ELEMENT_ARRAY_BUFFER expected as buffer target`);
            this._target = target;
        }
        return this._object;
    }
     ...
     */
    delete() {
        (0, auxiliaries_1.assert)(this._object instanceof WebGLBuffer, `expected WebGLBuffer object`);
    ...
    bind() {
        (0, auxiliaries_1.assert)(this._target === this._context.gl.ARRAY_BUFFER || this._target === this._context.gl.ELEMENT_ARRAY_BUFFER, `expected either ARRAY_BUFFER or ELEMENT_ARRAY_BUFFER as buffer target`);
    ...
    data(data, usage, bind = true, unbind = true) {
        ...
        const bytes = this._valid ? byteLength : 0;
     ...
     */
    @initializable_1.Initializable.assert_initialized()
    attribEnableInt(index, size, type, stride = 0, offset = 0, bind = true, unbind = true) {
        const gl = this.context.gl;
        if (bind) {
            this.bind();
        }
