...
class FrameCapture {
     ...
     */
    static createDataURL(imageData, type, quality) {
     ...
     */
    static captureDefaultFramebuffer(framebuffer, rect, buffer) {
     ...
     * @param buffer - Buffer to write the data into.
     */
    static captureFramebuffer(framebuffer, attachment, rect, buffer) {
        const gl = framebuffer.context.gl;
        const texture = framebuffer.texture(attachment);
        (0, auxiliaries_1.assert)(texture !== undefined, `Framebuffer does not have given attachment ${attachment}.`);
        (0, auxiliaries_1.assert)(texture.format === gl.RGBA, `Unsupported texture format ${texture.format}.`);
        (0, auxiliaries_1.assert)(texture.type === gl.UNSIGNED_BYTE || texture.type === gl.FLOAT || texture.type === gl.HALF_FLOAT, `Unsupported texture type ${texture.type}.`);
        ...
        if (texture.type === gl.FLOAT || texture.type === gl.HALF_FLOAT) {
            ...
            tempBuffer.forEach((value, index) => {
     ...
     */
    static flipImageDataVertically(imageData) {
     ...
     */
    static capture(framebuffer, attachment, rect) {
        const gl = framebuffer.context.gl;
        ...
        if (framebuffer instanceof defaultframebuffer_1.DefaultFramebuffer) {
            FrameCapture.captureDefaultFramebuffer(framebuffer, rect, buffer);
        }
        else if (framebuffer instanceof framebuffer_1.Framebuffer) {
