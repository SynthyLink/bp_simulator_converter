...
class CanvasRecorder {
     ...
     */
    constructor(canvas) {
        (0, auxiliaries_1.assert)(CanvasRecorder.isSupported(), 'Recording the canvas is not supported.');
        this._canvas = canvas;
     ...
     */
    start(fps, mimeType = 'video/webm', bitsPerSecond = 4 * 2 ** 20) {
        (0, auxiliaries_1.assert)(fps >= 0, 'FPS has to be positive');
        (0, auxiliaries_1.assert)(this._state === CanvasRecorder.State.INACTIVE, 'Recorder has to be inactive.');
        (0, auxiliaries_1.assert)(CanvasRecorder.isMIMETypeSupported(mimeType), `MIME type: ${mimeType} is not supported.`);
        this._stream = this._canvas.element.captureStream(fps);
        this._recorder = new MediaRecorder(this._stream, { mimeType, bitsPerSecond });
        this._images.length = 0;
        this._mimeType = mimeType;
        this._recorder.ondataavailable = (event) => this._images.push(event.data);
        this._recorder.onstop = () => { if (this._onImagesAvailable)
            this._onImagesAvailable(this._images); };
     ...
     */
    stop() {
        (0, auxiliaries_1.assert)(this._state !== CanvasRecorder.State.INACTIVE, 'Recorder must not be inactive.');
        const tracks = this._stream.getTracks();
        (0, auxiliaries_1.assert)(tracks.length === 1, 'CanvasRecorder uses outdated version of MediaStream API.');
        this._recorder.stop();
        tracks[0].stop();
     ...
     */
    frame() {
        (0, auxiliaries_1.assert)(this._state === CanvasRecorder.State.RECORDING, 'Recorder has to be recording.');
        const tracks = this._stream.getTracks();
        (0, auxiliaries_1.assert)(tracks.length === 1, 'CanvasRecorder uses unknown implementation of MediaStream API.');
        (0, auxiliaries_1.assert)(this._stream.requestFrame || tracks[0].requestFrame, 'CanvasRecorder uses unknown implementation of MediaStream API.');
     ...
     */
    set onImagesAvailable(callback) {
...
(function (CanvasRecorder) {
    ...
    })(State = CanvasRecorder.State || (CanvasRecorder.State = {}));
})(CanvasRecorder || (exports.CanvasRecorder = CanvasRecorder = {}));
