 ...
 */
class TextureCube extends object_1.AbstractObject {
    /**
    ...
    faceID(face) {
        ...
        switch (face) {
            ...
            case gl.TEXTURE_CUBE_MAP_NEGATIVE_Z:
                return 5;
            default:
                (0, auxiliaries_1.assert)(false, `expected texture cube map identifier (${gl.TEXTURE_CUBE_MAP_POSITIVE_X}, ` +
     ...
     */
    create(size, internalFormat, format, type) {
        (0, auxiliaries_1.assert)(size > 0, `texture cube requires valid size (width/height) of greater than zero`);
        const gl = this._context.gl;
     ...
     */
    cropImage(srcImage, offsetX, offsetY, width, height) {
        ...
        if (!ctx) {
            console.log(auxiliaries_1.LogLevel.Warning, '2D context creation failed when cropping image.');
     ...
     */
    extractMipLevelFromAtlas(image, mipLevel) {
     ...
     */
    @initializable_1.Initializable.assert_initialized()
    fetch(urisByFace, crossOrigin = false, mipLevel = 0) {
        const gl = this.context.gl;
            ...
            for (const tuple of images) {
                ...
                image.onload = () => {
                    ...
                    if (image.width !== image.height) {
                        (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `image ignored, width and height expected to be equal (square image)`);
                        return;
                    }
                    if (image.width !== size) {
                        (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `image ignored, width and height expected to match ` +
     ...
     */
    @initializable_1.Initializable.assert_initialized()
    fetchMipmapAtlas(uri, crossOrigin = false) {
        return new Promise((resolve, reject) => {
            const image = new Image();
            image.onerror = () => reject();
            image.onload = () => {
                if (image.width !== image.height * 2) {
                    (0, auxiliaries_1.log)(auxiliaries_1.LogLevel.Warning, `Mipmap atlas expected to have dimensions of 2x1.`);
                    return;
                }
                const mip0Size = image.height / 2;
     ...
     */
    @initializable_1.Initializable.assert_initialized()
    data(data, mipLevel = 0, bind = true, unbind = true) {
        const gl = this.context.gl;
        const gl2facade = this.context.gl2facade;
        const size = this.calculateMipLevelSize(mipLevel);
        let bytesPerFace = size * size * (0, formatbytesizes_1.byteSizeOfFormat)(this.context, this._internalFormat);
     ...
     */
    @initializable_1.Initializable.assert_initialized()
    levels(baseLevel, maxLevel, bind = true, unbind = true) {
        const gl = this.context.gl;
        if (bind) {
            this.bind();
        }
