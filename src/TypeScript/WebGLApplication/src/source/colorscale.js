...
class ColorScale {
     ...
     */
    static SCHEMA = require('./data/colorscalepresets.schema.json');
    /** @see{@link hint} */
    _hint = ColorScale.InterpolationHint.Linear;
    /** @ee{@link colors} */
    _colors = new Array();
    /** @see{@link invert} */
    _inverted = false;
     ...
     */
    static fromPreset(url, preset, stepCount) {
        ...
        return (0, fetch_1.fetchJsonAsync)(url, transform, ColorScale.SCHEMA.properties);
    }
    /**
     ...
     */
    static fromArray(interleavedColorComponents, type, stepCount, positions) {
        ...
        (0, auxiliaries_1.assert)(positions.length === colors.length, `expected number of positions (${positions.length}) to match number of colors (${colors.length})`);
        let lower = 0;
        let upper = lower + 1;
        const last = size - 1;
     ...
     */
    lerp(position, space = color_1.Color.Space.LAB) {
        ...
        const clamped = (0, gl_matrix_extensions_1.clamp)(position, 0.0, 1.0);
        if (clamped <= 0.0) {
            return this._colors[0];
        }
        else if (clamped >= 1.0) {
        ...
        (0, auxiliaries_1.assert)(upper < this._colors.length, `expected upper not exceed maximum color index: ${upper} < ${this._colors.length}`);
     ...
     */
    bitsUI8(space = color_1.Color.Space.RGB, alpha = true) {
     ...
     * @param alpha - Whether or not alpha is to be included.
     */
    bitsF32(space = color_1.Color.Space.RGB, alpha = true) {
        const size = this._colors.length;
        const stride = alpha ? 4 : 3;
