...
/* spellchecker: enable */
let types;
const byteSizesByFormatAndContext = new Map();
/**
 ...
 */
function isType(context, format) {
    const gl = context.gl;
    ...
    if (types === undefined) {
        ...
        types = new Array(gl.UNSIGNED_BYTE, gl.UNSIGNED_SHORT_5_6_5, gl.UNSIGNED_SHORT_4_4_4_4, gl.UNSIGNED_SHORT_5_5_5_1, gl.UNSIGNED_SHORT, gl.UNSIGNED_INT, UNSIGNED_INT_24_8_WEBGL, gl.FLOAT, gl2facade.HALF_FLOAT, gl.BYTE, gl.SHORT, gl.INT, gl.HALF_FLOAT, gl.UNSIGNED_INT_2_10_10_10_REV, gl.UNSIGNED_INT_10F_11F_11F_REV, gl.UNSIGNED_INT_5_9_9_9_REV, gl.UNSIGNED_INT_24_8, gl.FLOAT_32_UNSIGNED_INT_24_8_REV);
    }
...
function byteSizeOfFormat(context, format) {
    const gl = context.gl;
    if (format === undefined) {
        (0, auxiliaries_1.assert)(false, `expected given format to be defined`);
        return 0;
    }
    if (isType(context, format)) {
        (0, auxiliaries_1.assert)(false, `expected format instead of type ${format}`);
        return 0;
    }
    if (format === gl.DEPTH_COMPONENT || format === gl.DEPTH_STENCIL) {
        (0, auxiliaries_1.assert)(false, `byte size of DEPTH_COMPONENT or DEPTH_STENCIL formats depends on active render buffer`);
        return 0;
    }
    const byteSizesByFormat = byteSizes(context);
    const result = byteSizesByFormat.get(format);
    (0, auxiliaries_1.assert)(result !== undefined, `size of format ${format} is unknown`);
    return result ? result : 0;
}
//# sourceMappingURL=formatbytesizes.js.map