...
describe('Tile Camera Generator Hilbert Iteration', () => {
    it(' should render in the correct order', () => {
        const algorithm = tilecameragenerator_1.TileCameraGenerator.Algorithm.HilbertCurve;
        const tileDimensions = [7, 5];
...
describe('Tile Camera Generator Z-Curve Iteration', () => {
    it(' should render in the correct order', () => {
        const algorithm = tilecameragenerator_1.TileCameraGenerator.Algorithm.ZCurve;
        const tileDimensions = [7, 5];
        // generate the expected order
        const expectedOrder = [
...
function createTileCameraGenerator(tileDimensions, algorithm = tilecameragenerator_1.TileCameraGenerator.Algorithm.ScanLine) {
    const camera = new camera_1.Camera(gl_matrix_1.vec3.fromValues(0, 0, 0), gl_matrix_1.vec3.fromValues(1, 0, 0), gl_matrix_1.vec3.fromValues(0, 0, 1));
    const tileCameraGenerator = new tilecameragenerator_1.TileCameraGenerator();
    tileCameraGenerator.sourceCamera = camera;
    tileCameraGenerator.padding = gl_matrix_1.vec4.create();
...
function testTileCameraIndicesMatchExpected(expectedOrder, tileDimensions, algorithm) {
...
describe('TileCameraGenerator.reset', () => {
    ...
    it('should set the offset to 0', () => {
        const tileCameraGenerator = new tilecameragenerator_1.TileCameraGenerator();
        tileCameraGenerator.padding = gl_matrix_1.vec4.fromValues(1, 1, 1, 1);
        tileCameraGenerator.reset();
...
describe('TileCameraGenerator.sourceCameraChanged', () => {
    it('should set the camera', () => {
        const tileCameraGenerator = createTileCameraGenerator([8, 8]);
        const newEyePosition = gl_matrix_1.vec3.fromValues(1, 0, 0);
...
//# sourceMappingURL=tilecamgeragenerator.test.js.map