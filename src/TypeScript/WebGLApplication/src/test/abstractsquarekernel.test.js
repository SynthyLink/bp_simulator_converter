...
const kernel_1 = require("../source/kernel");
describe('PrimitiveKernels', () => {
    it('function position to never fail and return undefined', () => {
        const kernel32fArray = [new kernel_1.KernelF32(1, 1), new kernel_1.KernelF32(2, 2, 2), new kernel_1.KernelF32(4, 4, 4, 4)];
    ...
    it('get function should run on correct indices', () => {
        const kernel32f1 = new kernel_1.KernelI8(1, 1);
        const kernel32f2 = new kernel_1.KernelI8(2, 2, 2);
        const kernel32f3 = new kernel_1.KernelI8(3, 3, 3, 3);
        const kernel32f4 = new kernel_1.KernelI8(4, 4, 4, 4);
        for (let x = 0; x < 4; x++) {
            for (let y = 0; y < 4; y++) {
                for (let z = 0; z < 4; z++) {
    ...
    it('set function should run on correct indices', () => {
        const kernel32f1 = new kernel_1.KernelUI8(1, 1);
        const kernel32f2 = new kernel_1.KernelUI8(2, 2, 2);
        const kernel32f3 = new kernel_1.KernelUI8(3, 3, 3, 3);
        const kernel32f4 = new kernel_1.KernelUI8(4, 4, 4, 4);
        for (let x = 0; x < 4; x++) {
            for (let y = 0; y < 4; y++) {
                for (let z = 0; z < 4; z++) {
    ...
    it('should be sortable with evey sorting approach', () => {
        const kernel32fArray = [new kernel_1.KernelI32(1, 1), new kernel_1.KernelI32(2, 2, 2), new kernel_1.KernelI32(4, 4, 4, 4)];
        expect(kernel32fArray).to.not.be.undefined;
        kernel32fArray.forEach((array) => {
            array.sort(kernel_1.AbstractKernel.SortApproach.BySquaredLength);
        });
    });
    it('should be initializable from JSON', () => {
        let kernelJson = { kernel: new Array(1), size: { width: 1, height: 1, depth: 1 } };
        const kernelF32 = new kernel_1.KernelF32(1, 1);
        expect(() => { kernelF32.fromJSON(kernelJson); }).to.not.throws();
        kernelJson = { kernel: new Array(1), size: { width: 1, height: 1, depth: 1 } };
        const kernelUI32 = new kernel_1.KernelUI32(1, 1);
        expect(() => { kernelUI32.fromJSON(kernelJson); }).to.not.throws();
        kernelJson = { kernel: new Array(1), size: { width: 1, height: 1, depth: 1 } };
        const kernelI32 = new kernel_1.KernelI32(1, 1);
        expect(() => { kernelI32.fromJSON(kernelJson); }).to.not.throws();
        kernelJson = { kernel: new Array(1), size: { width: 1, height: 1, depth: 1 } };
        const kernelUI8 = new kernel_1.KernelUI8(1, 1);
        expect(() => { kernelUI8.fromJSON(kernelJson); }).to.not.throws();
    });
    it('should be able to retrieve dimension lengths', () => {
        const kernelF32 = new kernel_1.KernelI32(1, 1);
...
//# sourceMappingURL=abstractsquarekernel.test.js.map