...
export class AtmospherePure {
    ...
    mac: number[] = [31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365];
    f01: number[] = [
    ...
    KDNEY: number[] = [31, 0, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];

    N10: number = 0;
    f1: number[] = [0];
    ifa: number[]=[150, 6, 140 ];
    if1: number[] = [ 75, 100, 125, 150, 200, 250 ];
date: number[] = [0, 0, 0];
r : number[] =[0, 0, 0];
    private y: number[] =[0, 0, 0];
    private ff0: number[][]= [[0]];
    private ff1: number[][] = [[0]];
    ome: number = 7.292115085E-5;
    ...
    AngleSun(T: number, D: number, ASoL: number[], DSoL: number[]): void
{
    ...
    shiftdat(dat: DateTime): number {
        ...
        for (let i = 1; i < dat.month; i++) data += this.KDNEY[i];
        return data;
    }

    zvvr(D: number): number{
            let T1 = D / 36525.0;
            let T2 = T1 * T1;
