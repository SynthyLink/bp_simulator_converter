import {
	AlwaysDepth, EqualDepth, GreaterDepth, GreaterEqualDepth,
	LessDepth, LessEqualDepth, NeverDepth, NotEqualDepth
} from './constants';
 ...
 */
function getTypedArray(type: string, buffer) {
 ...
 */
function enhanceLogMessage( params : any[]) : any[]{
 ...
 */
function probeAsync(gl: WebGL2RenderingContext, sync: WebGLSync, interval: number): Promise<void>{
 ...
 */
function toNormalizedProjectionMatrix(projectionMatrix: Matrix4): void{
 ...
 */
function toReversedProjectionMatrix(projectionMatrix: Matrix4 ) : void{
...
function yieldToMain(): Promise<void> {
	...
	return new Promise(resolve => {
		let r = resolve as undefined as FrameRequestCallback
		if (r != undefined)
