...
export class RealMatrix {
    ...
    public partialNorm(x: number[], startIndex: number, length: number): number {
        return Math.sqrt(this.partialSquare(x, startIndex, length));
    }


    public plusEqual(x: number[], y: number[]): void {
    ...
    public setLength(x: number[], n: number): void {
        x.fill(0, 0, n - 1);
    }


    public setLength2(x: number[][], n: number, m: number): void {
        for (let i = 0; i < n; i++) {
            let y: number[] = [];
            this.setLength(y, m);
            x.push(y)
    ...
    public normalize(inp: number[], outp: number[], offset: number) {
        let a = 0;
    ...
    public getNorm(vector: number[]) {
        return Math.sqrt(this.square(vector));
    }

    public copySign(a: number, b: number): number {
        return Math.abs(a) * Math.sign(b);
    }

    public invertA(a: number[][]): number[][] {
        let x: number[][] = [];
        var n = a.length;
    ...
    public invert(a: number[][], aInverted: number[][]): void {
        let e: number = 0;
        let y: number = 0;
        let det: number = 0;
        let w: number = 0;
    ...
    public scalarProduct(x: number[], y: number[]): number {
        let sum = 0;
        for (let i = 0; i < x.length; i++) {
            sum += x[i] * y[i];
        }
    ...
    public multiply(vector: number[], coefficient: number) {
        for (let i = 0; i < vector.length; i++) {
            vector[i] = vector[i] * coefficient;
        }
    }

    public multiplyMatrix(a: number[][], b: number[][], c: number[][]): void {
        let a1 = a.length;
        let a2 = a[0].length;
        let b1 = b.length;
        let b2 = b[0].length;
    ...
    public square(vec: number[]): number {
        let a = 0;
        for (let x of vec) {
            a += x * x;
        }
    ...
    public multiplyRight(matrix: number[][], vector: number[], product: number[]): void {
    ...
    public multiplyLeft(vector: number[], matrix: number[][], product: number[]): void {
    ...
    public transpose(x: number[][], y: number[][]) {
        for (let i = 0; i < x.length; i++) {
    ...
    public htah(h: number[][], a: number[][], result: number[][]): void {
    ...
    public addMatrix(x: number[][], y: number[][], z: number[][]): void {
        for (let i = 0; i < x.length; i++) {
            for (let j = 0; j < x[0].length; j++) {
                z[i][j] = x[i][j] + y[i][j];
            }
    ...
    public addVector(x: number[], y: number[], z: number[]): void {
        for (let i = 0; i < x.length; i++) {
    ...
    public diffatrix(x: number[][], y: number[][], z: number[][]): void {
        for (let i = 0; i < x.length; i++) {
            for (let j = 0; j < x[0].length; j++) {
                z[i][j] = x[i][j] - y[i][j];
            }
    ...
    public diffVector(x: number[], y: number[], z: number[]): void {
        for (let i = 0; i < x.length; i++) {
    ...
    public lu_Factor(A: number[][], indx: number[]) {
            let i = 0, j = 0, k = 0;
            let jp = 0;
            let t = 0;
        let M = A.length;
    ...
    public lu_Solve(A: number[][], indx:  number[],  b: number[]): boolean {
            let i, ii = -1, ip = 0, j;
            let n = b.length;
            let sum = 0;
    ...
    public solve(a: number[][], indx: number[], b: number[]): boolean {
