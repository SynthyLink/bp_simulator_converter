...
namespace DataPerformer.Formula.Python
{
	...
	internal class TreeCodeCreator : SeparatorCodeCreator, IOperationSeparatorCreator
    {
        ...
        new Dictionary<string, string[]> {
            ...
            {"e", new string[] {" = np.exp(", ")"}},
            {"t", new string[] {" = np.tan(", ")"}},
            {"q", new string[] {" = np.tan( Math.PI / 2 - (", "))"}},
            {"a", new string[] {" = np.atan(", ")" }},
            {"b", new string[] {" = np.pi / 2 - np.atan(", ")" }},
            {"j", new string[] {" = 1 / np.cos(", ")" }},
            {"k", new string[] {" = 1 / np.sin(", ")" }},
            {"f", new string[] {" = np.asin(", ")" }},
            {"g", new string[] {" = np.acos(", ")" }},
            {"?", new string[] {" = (", ")" }},
            {"-", new string[] {" = -(", ")" }},
            {"A", new string[] {" = np.abs(", ")" }},
        };

        private static readonly string[] squareRoot = new string[] { " = np.sqrt(", ")" };

        private static readonly string[] root = new string[] { " = np.power(", ", 1 /(", "))" }; //TODO replace with **

        private static readonly string[] abs = new string[] { " = np.abs(", ")" };

        private static readonly string[] absPower = new string[] { " = np.power(Math.abs(", "), ", ")" }; // TODO replac with **

        private static readonly string[] atan2 = new string[] { " = np.atan2(", ", ", ")" };
        ...
        private static readonly string[] power = new string[] { " = np.power(", ", ", ")" };
        ...
        private List<string> CreatePythonCode(object obj, ObjectFormulaTree tree, string ret, string[] parameters,
            ...
            string ret, string[] parameters)
        {
            ...
            IList<string> l = CreatePythonCode(obj, tree, ret, parameters, out variables, out initializers);
            if (l != null)
            {
